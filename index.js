    ...
    css.walkRules(function (rule) {
      rule.walkDecls(function (decl, i) {
        ...
        if (value.indexOf( 'url(' ) !== -1) {
          ...
          var strippedInputFileName = path.basename(result.opts.from).replace(extensionPattern, '');
          ...
          var inputFilePath = path.dirname(result.opts.from);

          // Path from root to file. Something like /opt/my/project/path/image.png
          var rootPathToFile = path.resolve(inputFilePath, urlPath);
          if (!fs.existsSync(dumpDirectoryPath)) {
            fs.mkdirSync(dumpDirectoryPath)
          }

          // Copy the file to our static directory
          var inStr = fs.createReadStream(rootPathToFile);
          var outStr = fs.createWriteStream(dumpDirectoryPath + "/" + baseUrlFileName);
          inStr.pipe(outStr);

          // Send the new value back into the stylesheet
          decl.value = value.replace(urlPattern, 'url("' + newUrlPathPrefix + "/" + baseUrlFileName + '")');
        }
      });
    });
  }
